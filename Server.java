...
private void clearInputBuffer() throws IOException {
    while (in.ready()) {
        ...
        private List<Map<String, Object>> fetchRandomQuestions(int challengeNo) throws SQLException {
            String questionSql = "SELECT q.questionNo, q.question, a.answer, a.marksAwarded FROM Questions q JOIN Answer a ON q.questionNo = a.questionNo WHERE q.questionBankID = (SELECT questionBankID FROM Challenge WHERE challengeNo = ?) ORDER BY RAND() LIMIT 10";
